...
(function() {
    ...
    var chatUrl = function() {
        ...
        return SP.chatPath + "client-chat-page.html?"
            ...
            + location
            + "&phone_number=" + encodeURIComponent(SP.phone_number || '')
			+ "&from=" + encodeURIComponent(SP.from || '')
            + "&email=" + encodeURIComponent(SP.email || '')
            + "&account_number=" + encodeURIComponent(SP.account_number || '')
            + "&last_name=" + encodeURIComponent(SP.last_name || '')
            + "&first_name=" + encodeURIComponent(SP.first_name || '')
            + "&togetherJS_enabled=" + encodeURIComponent(SP.togetherJS_enabled || '');
    };
    ...
    var openChat = function(open, deleteIframe) {
        ...
        if(navigator.userAgent.indexOf("Safari") != -1 && navigator.userAgent.indexOf('Chrome') == -1) {
            fakeFrom.hide();
            fakeTo.toggle();
            whenIframeIsOpened(deleteIframe);
        } else {
            fakeTo.toggle();
    ...
    var whenIframeIsOpened = function(deleteIframe) {
        init = true;
        if(deleteIframe) {
            $('#sp-chat-iframe').remove();
        } else {
    ...
    var setFollowPeersTogetherJSTask = null;

    var setFollowPeersTogetherJS = function() {
    	if (TogetherJS.running) {
    		TogetherJS.require("peers").getAllPeers().forEach(function (p) {
    			if (!p.following) {
    				p.follow();
    			}
    		});
    ...
    var TogetherJSUrlWasSent = false;

    var startTogetherJS = function() {
    	if (!TogetherJS.running) {
    		TogetherJS();
    		if (!TogetherJSUrlWasSent) {
    			var url = null;
    			try {
...
//    var streamTimer;
    ...
    if (SP.tenantUrl && SP.appId) {
        ...
        loadScripts(['js/json2.min.js', 'js/jquery-1.11.0.min.js', 'js/togetherjs-min.js'], function() {
